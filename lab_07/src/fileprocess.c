...
err_t read_ohash_from_file(ohash_table_t *table, const char *filename, bool info) {
    ...
    for (;!err && !is_eof(file);) {
        err = word_read(file, &word);
        if (!err) {
            ohash_add(table, word, info);
            cnt++;
        }
        free(word);
    }
    fclose(file);
    return !err ? cnt ? OK : ERR_EMPTY_FILE : err;
}
...
#define MAX_PYTHON_CALL 1000
    ...
    if (str_cnt <= 0 || str_cnt > 20) return ERR_IO;
